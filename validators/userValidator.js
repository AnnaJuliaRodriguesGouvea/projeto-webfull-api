    ...
    validateEmail: function(req, res, next) {
        ...
        if (error) {
            if (error.details && error.details[0].type === 'string.empty') {
              return res.status(400).json({ status: false, data: "O email não pode ser nulo" });
            }
        
            if (error.details && error.details[0].type === 'string.email') {
              return res.status(400).json({ status: false, data: "O email não está no formato correto" });
            }
        
            return res.status(400).json({ status: false, data: "Erro na validação do email" });
        }
    ...
    validatePassword: function(req, res, next) {
        ...
        if (error) {
            ...
            if (error.details && error.details[0].type === 'string.min') {
                return res.status(400).json({ status: false, data: "A senha deve ter pelo menos 6 caracteres" });
            }
        
            if (error.details && error.details[0].type === 'string.pattern.base') {
                return res.status(400).json({ status: false, data: "A senha não atende aos requisitos mínimos" });
            }
